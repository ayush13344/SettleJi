...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ep-group-img {
    ...
    box-shadow: 0 2px 8px rgba(79,70,229,.18); flex-shrink: 0;
  }
  ...
  .ep-hero-rs {
    font-family: 'Fraunces', serif;
    font-size: 28px; font-weight: 900; color: rgba(255,255,255,.75); line-height: 1;
  }
  .ep-hero-input {
    font-family: 'Fraunces', serif;
    font-size: 52px; font-weight: 900;
    color: #fff; background: none; border: none; outline: none;
    width: 210px; letter-spacing: -3px; line-height: 1;
  ...
  .ep-input:focus, .ep-textarea:focus {
    border-color: var(--indigo); box-shadow: 0 0 0 3px rgba(79,70,229,.1); background: var(--white);
  }
  ...
  .ep-cat-item:hover {
    border-color: var(--indigo); background: var(--indigo-lt); color: var(--indigo);
    transform: translateY(-2px); box-shadow: 0 4px 12px rgba(79,70,229,.15);
  }
  .ep-cat-item.sel {
    border-color: var(--indigo); background: var(--indigo-lt); color: var(--indigo);
  ...
  /* ─── UPLOAD ─── */
  .ep-upload {
    margin: 0 16px 4px; border: 2px dashed var(--indigo-md);
    border-radius: 16px; padding: 18px 16px;
    background: var(--indigo-lt); text-align: center;
    cursor: pointer; transition: all .18s; display: block;
  }
  .ep-upload:hover {
    border-color: var(--indigo); background: #e0e7ff;
    transform: translateY(-2px); box-shadow: 0 6px 18px rgba(79,70,229,.12);
  }
  ...
  .ep-upload-preview {
    ...
    object-fit: cover; max-height: 190px; border: 2px solid var(--indigo-md);
  }

  /* ─── PAID BY ─── */
  .ep-payer-scroll {
    display: flex; gap: 12px; padding: 4px 16px 20px; overflow-x: auto;
  }
  ...
  .ep-av.sel {
    ...
    box-shadow: 0 0 0 3px rgba(79,70,229,.25), 0 4px 14px rgba(0,0,0,.18);
  }
  .ep-av-check {
  ...
  .ep-split-row {
    ...
    background: #fafafe; transition: background .15s;
  }
  ...
  .ep-split-share {
    font-family: 'Fraunces', serif;
    font-size: 16px; font-weight: 900; color: var(--indigo);
  }
  .ep-split-tag {
    font-size: 10px; font-weight: 700; padding: 2px 8px; border-radius: 6px;
    background: #dcfce7; color: #16a34a; margin-left: 6px;
  }
  .ep-split-tag.payer { background: #ede9fe; color: var(--indigo); }

  /* ─── SUMMARY CARD ─── */
  .ep-summary {
    margin: 0 16px 8px; border-radius: 18px; overflow: hidden;
    border: 1.5px solid var(--border); box-shadow: 0 4px 16px rgba(79,70,229,.08);
  }
  .ep-summary-header {
  ...
  .ep-summary-body {
    background: #fafafe; padding: 14px 18px;
    display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px;
  }
  .ep-sum-stat { text-align: center; }
  .ep-sum-val {
    font-family: 'Fraunces', serif; font-size: 18px; font-weight: 900; color: var(--ink); margin-bottom: 2px;
  }
  ...
  .ep-cta-btn:hover:not(:disabled) {
    transform: translateY(-2px); box-shadow: 0 12px 32px rgba(79,70,229,.5);
  }
...
export default function ExpensePage() {
  ...
  const handleAddExpense = async () => {
    try {
      ...
      // ✅ FIX: correctly calculate each person's share
      ...
      const payload = {
        ...
        amount:   totalAmount,
        category,
        // ✅ store name as paidBy — group members have no _id
        paidBy:   members[selectedPayer]?.name,
        participants: members.map(m => ({
          // ✅ store name as user — matched by name in balance calculation
          user:  m.name,
      ...
      console.log("Total:", totalAmount, "| Members:", members.length, "| Share each:", sharePerPerson);
      ...
      <div className="ep-root">
        <div className="ep-shell">
          ...
          <div className="ep-group-banner">
            <img
              src={group?.coverImage || "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=120&q=60"}
              alt="group" className="ep-group-img"
            />
            <div>
          ...
          <div className="ep-hero">
            ...
            <div className="ep-hero-amt-row">
              <div className="ep-hero-amt-left">
                ...
                <input
                  ...
                  className="ep-hero-input" placeholder="0"
                />
              </div>
            ...
            <div className="ep-quick-row">
              {[100, 200, 500, 1000, 2000, 5000].map(n => (
                <button key={n} className="ep-quick-chip" onClick={() => setAmount(a => a + n)}>
                  +₹{n}
                </button>
              ))}
            </div>
          ...
          <div className="ep-form">
            <div className="ep-field-wrap">
              ...
              <input
                className="ep-input" placeholder="What's this expense for?"
            ...
            <div className="ep-field-wrap">
              ...
              <textarea
                className="ep-textarea" style={{paddingTop:13}}
          ...
          <div className="ep-cat-grid">
            {CATS.map(cat => (
              <div key={cat.v} className={`ep-cat-item${category === cat.v ? " sel" : ""}`}
                onClick={() => setCategory(cat.v)}>
                <span className="ci">{cat.e}</span>
                {cat.v}
              </div>
            ))}
          </div>
          ...
          <label className="ep-upload">
            {preview ? (
              ...
              <>
                ...
                <div className="ep-upload-title">Tap to upload bill</div>
                <div className="ep-upload-sub">JPG, PNG or PDF supported</div>
              </>
            )}
            ...
            <div className="ep-summary" style={{marginTop:12}}>
              ...
              <div className="ep-summary-payer">
                ...
                <div style={{marginLeft:"auto", textAlign:"right"}}>
                  ...
                  <div className="ep-sum-payer-name">
